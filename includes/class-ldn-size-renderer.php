...
final class LDN_Size_Renderer {
    ...
    /** @var bool Plotly CDN script tag already output on this page. */
    private $plotly_script_emitted = false;
    ...
    public function render(LDN_Page_Context $ctx, array $summary) {
        $this->plotly_script_emitted = false;
        ...
        if ($ctx->page_level === 'size-comparison') {
            $out .= $this->comparison_price_links_html($ctx, $summary);
        } else {
    ...
    private function individual_body_html(LDN_Page_Context $ctx, array $summary, array $copy = array()) {
        ...
        $all_charts = $this->fetch_distribution_charts($ctx);
        $spread_charts = $this->filter_charts_by_keys(
            $all_charts,
            $this->spread_chart_keys($summary)
        );
        $distribution_charts = $this->filter_charts_by_keys(
            $all_charts,
            $this->distribution_chart_keys($summary)
        );
        ...
        if ($spread_svg !== '') {
            ...
            $spread = '<section class="ldn-section ldn-size-spread ldn-size-spread--primary"><h2>'
                ...
                . '</figcaption></figure>'
                . $this->render_size_chart_blocks($ctx, $spread_charts, array(
                    'on_purple' => false,
                    'heading_as' => 'h3',
                ))
                . '</section>';
        ...
        $spread_overflow = '';
        if ($spread_svg === '' && $spread_charts !== array()) {
            $spread_overflow = $this->distribution_chart_section_html(
                $ctx,
                $summary,
                $spread_charts,
                true
        ...
        $tail = $spread_overflow
            . $this->size_segmentation_html($summary, $ctx)
            . $this->distribution_chart_section_html($ctx, $summary, $distribution_charts)
            . ($full_range_site ? '' : $this->chart_vs_real_html($summary, $ctx->site_id));
    ...
    public function factual_fallback(array $summary) {
        ...
        if (isset($summary['type']) && $summary['type'] === 'shape_hub') {
            $shape_label = isset($summary['shape'])
                ? ucwords(str_replace('-', ' ', (string) $summary['shape']))
                : __('Diamond', 'loupe-diamond-network');
            $total_n = isset($summary['total_n']) ? (int) $summary['total_n'] : 0;
            if ($total_n > 0) {
                return sprintf(
                    /* translators: 1: shape name, 2: formatted diamond count, 3: shape name (lowercase) */
                    __(
                        'Median %1$s sizes below are based on %2$s real %3$s diamonds measured in retailer inventory.',
                        'loupe-diamond-network'
                    ),
                    $shape_label,
                    number_format($total_n),
            ...
            return sprintf(
                ...
                __(
                    'Median %s sizes below are based on real diamonds measured in retailer inventory.',
                    'loupe-diamond-network'
                ),
                $shape_label
            );
        }
        if (isset($summary['type']) && $summary['type'] === 'mega_hub') {
    ...
    /**
     * Legacy entry point: renders all distribution charts in one section (hubs / tests).
     *
     * @param LDN_Page_Context $ctx
     * @return string
     */
    public function chart_html(LDN_Page_Context $ctx, array $summary = array(), $site_id = null) {
        $charts = $this->fetch_distribution_charts($ctx);
        if ($charts === array()) {
            return '';
        }
        return $this->distribution_chart_section_html($ctx, $summary, $charts, true);
    }
     ...
     */
    private function fetch_distribution_charts(LDN_Page_Context $ctx) {
        ...
        if (isset($payload['data'])) {
            return array(array('key' => 'primary', 'title' => '', 'figure' => $payload));
        }
        return array();
    }
     ...
     * @return string[]
     */
    private function spread_chart_keys(array $summary) {
        return $this->is_near_round($summary) ? array('diameter') : array('length');
    }
     ...
     * @return string[]
     */
    private function distribution_chart_keys(array $summary) {
        return $this->is_near_round($summary) ? array() : array('lw_ratio');
    }
     ...
     * @return array<int, array<string, mixed>>
     */
    private function filter_charts_by_keys(array $charts, array $keys) {
        if ($keys === array()) {
            return array();
        }
        ...
        foreach ($charts as $chart) {
            ...
            $key = isset($chart['key']) ? (string) $chart['key'] : '';
     ...
     * @param array                               $summary
     * @param array<int, array<string, mixed>>    $charts
     * @param bool                                $legacy_all_in_one When true, render every chart in one white section.
     * @return string
     */
    public function distribution_chart_section_html(
        ...
        $elongated = !$this->is_near_round($summary) && !$legacy_all_in_one;
        $section_class = 'ldn-section ldn-chart ldn-size-chart';
        if ($elongated) {
            $section_class .= ' ldn-section--tint ldn-size-chart--on-purple';
        }

        $out = '<section class="' . esc_attr($section_class) . '">';
        ...
        if ($legacy_all_in_one) {
            ...
            $out .= $this->render_size_chart_blocks($ctx, $charts, array(
                'on_purple' => false,
                'heading_as' => count($charts) > 1 ? 'h3' : 'none',
            ));
        } else {
            $first = $charts[0];
            ...
            $out .= '<p class="ldn-size-chart__lead">' . esc_html($this->lw_ratio_explainer_text()) . '</p>';
            $out .= $this->render_size_chart_blocks($ctx, $charts, array(
                'on_purple' => true,
                'heading_as' => 'none',
            ));
     ...
     * @param LDN_Page_Context                 $ctx
     * @param array<int, array<string, mixed>> $charts
     * @param array<string, mixed>             $options on_purple (bool), heading_as (none|h2|h3)
     * @return string
     */
    private function render_size_chart_blocks(LDN_Page_Context $ctx, array $charts, array $options) {
        if ($charts === array()) {
            return '';
        }
        $on_purple = !empty($options['on_purple']);
        $heading_as = isset($options['heading_as']) ? (string) $options['heading_as'] : 'h3';

        $out = '';
        if (!$this->plotly_script_emitted) {
            $out .= '<script src="' . esc_url(self::PLOTLY_CDN) . '"></script>';
            $this->plotly_script_emitted = true;
        }
        ...
        foreach ($charts as $idx => $chart) {
            ...
            $figure = $this->prepare_size_chart_figure($chart['figure'], $on_purple);
            $fig_json = wp_json_encode($figure, self::JSON_SCRIPT_FLAGS);
            if (!is_string($fig_json) || $fig_json === '') {
                continue;
            }
            if ($heading_as !== 'none' && $sub !== '') {
                $tag = $heading_as === 'h2' ? 'h2' : 'h3';
                $out .= '<' . $tag . ' class="ldn-size-chart__subheading">' . esc_html($sub) . '</' . $tag . '>';
            }
            $target_class = 'ldn-chart-target';
            if ($on_purple) {
                $target_class .= ' ldn-chart-target--on-purple';
            }
            $out .= '<div id="' . esc_attr($id) . '" class="' . esc_attr($target_class) . '"></div>';
    ...
    /**
     * @param array<string, mixed> $figure Plotly figure payload from size-distribution.json.
     * @param bool                 $on_purple When true, recolour bars and axes for a purple band.
     * @return array<string, mixed>
     */
    private function prepare_size_chart_figure(array $figure, $on_purple) {
        if (!$on_purple) {
            return $figure;
        }
        $white = '#ffffff';
        $fig = $figure;
        if (!empty($fig['data']) && is_array($fig['data'])) {
            foreach ($fig['data'] as $i => $trace) {
                if (!is_array($trace)) {
                    continue;
                }
                if (!isset($fig['data'][$i]['marker']) || !is_array($fig['data'][$i]['marker'])) {
                    $fig['data'][$i]['marker'] = array();
                }
                $fig['data'][$i]['marker']['color'] = $white;
                $fig['data'][$i]['marker']['line'] = array('color' => $white, 'width' => 0.5);
            }
        }
        if (!isset($fig['layout']) || !is_array($fig['layout'])) {
            $fig['layout'] = array();
        }
        $fig['layout']['paper_bgcolor'] = 'rgba(0,0,0,0)';
        $fig['layout']['plot_bgcolor'] = 'rgba(0,0,0,0)';
        $fig['layout']['font'] = array_merge(
            isset($fig['layout']['font']) && is_array($fig['layout']['font']) ? $fig['layout']['font'] : array(),
            array('color' => $white)
        );
        foreach (array('xaxis', 'yaxis') as $axis) {
            $existing = isset($fig['layout'][$axis]) && is_array($fig['layout'][$axis])
                ? $fig['layout'][$axis]
                : array();
            $tickfont = isset($existing['tickfont']) && is_array($existing['tickfont'])
                ? $existing['tickfont']
                : array();
            $titlefont = isset($existing['titlefont']) && is_array($existing['titlefont'])
                ? $existing['titlefont']
                : array();
            $fig['layout'][$axis] = array_merge($existing, array(
                'tickfont' => array_merge($tickfont, array('color' => $white)),
                'titlefont' => array_merge($titlefont, array('color' => $white)),
     ...
     * @return string
     */
    private function lw_ratio_explainer_text() {
        return __(
    ...
    public function ideal_vs_real_html(array $summary) {
        ...
        if ($this->is_near_round($summary)) {
            ...
            $text = sprintf(
                ...
                __(
                    'Other websites that publish diamond size charts often quote a %1$s carat %2$s at %5$s x %6$s mm (about %7$s mm diameter). '
            ...
            $text = sprintf(
                ...
                __(
                    'Other websites that publish diamond size charts often quote a %1$s carat %2$s at %5$s x %6$s mm. '
                    . 'Our inventory median is %3$s x %4$s mm.',
                    'loupe-diamond-network'
                ),
                $carat,
     ...
     * the depth↔face-up correlation narrative, and the about-this-data strip.
     *
     * @param array       $summary
     * @param string|null $site_id
     * @return string
     */
    public function chart_vs_real_html(array $summary, $site_id = null) {
        $ideal = $this->ideal_vs_real_html($summary);
        $depth = $this->proportions_html($summary);
        $methodology = ($site_id !== null) ? $this->methodology_inline_html($summary, $site_id) : '';
        if ($ideal === '' && $depth === '' && $methodology === '') {
            return '';
        }
    ...
    public function hub_table_html(LDN_Page_Context $ctx, array $summary) {
        ...
        $show_carat_col = in_array($ctx->page_level, array('size-mega-hub', 'size-shape-hub'), true);
        $rows = isset($summary['rows']) && is_array($summary['rows']) ? $summary['rows'] : array();
        if ($rows === array()) {
            return '';
        }
        ...
        foreach ($rows as $probe) {
            ...
            if (isset($probe['depth_pct']) && $probe['depth_pct'] !== null && $probe['depth_pct'] !== '') {
                $has_depth = true;
            }
        ...
        foreach ($rows as $row) {
            ...
            $body .= '<tr' . ($url !== '' ? ' class="ldn-size-table__row--linked"' : '') . '>';
            if ($show_shape) {
                $shape_cell = $url !== ''
            ...
            if ($show_carat_col) {
                $carat_label = $carat !== '' ? $carat . ' ct' : '';
                $carat_cell = ($url !== '' && $carat_label !== '')
                    ? '<a href="' . esc_url($url) . '">' . esc_html($carat_label) . '</a>'
                    : esc_html($carat_label);
                $body .= '<td>' . $carat_cell . '</td>';
            }
            if ($has_visual) {
                $outline_class = $ctx->page_level === 'size-shape-hub'
                    ? 'ldn-size-matrix__outline'
                    : 'ldn-size-table-thumb';
                $thumb = (!empty($row['outline_svg']) && is_string($row['outline_svg']))
                    ? '<span class="' . esc_attr($outline_class) . '">' . $row['outline_svg'] . '</span>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                    : '';
                if ($url !== '' && $thumb !== '') {
                    $link_class = $ctx->page_level === 'size-shape-hub'
                        ? 'ldn-size-matrix__link'
                        : 'ldn-size-table-thumb-link';
                    $thumb = '<a href="' . esc_url($url) . '" class="' . esc_attr($link_class) . '">'
                        . $thumb . '</a>';
                }
                $thumb_cell_class = $ctx->page_level === 'size-shape-hub'
                    ? 'ldn-size-matrix__cell'
                    : 'ldn-size-table-thumb-cell';
                $body .= '<td class="' . esc_attr($thumb_cell_class) . '">' . $thumb . '</td>';
            }
            ...
            if ($has_depth) {
                $depth_val = $row['depth_pct'] ?? null;
                $depth_txt = ($depth_val !== null && $depth_val !== '')
                    ? (string) $depth_val
                    : '—';
                $body .= '<td>' . esc_html($depth_txt) . '</td>';
            }
            $body .= '</tr>';
        }
        if ($body === '') {
        ...
        $lead = '';
        if ($ctx->page_level === 'size-shape-hub') {
            $lead = '<p class="ldn-size-chart__lead">' . esc_html__(
                'Median length, width, and face-up area at each carat weight. Click a row to see how much stones vary at that weight.',
                'loupe-diamond-network'
            ) . '</p>';
        }
    ...
    /**
     * Three-step explainer for the full Diamond Size Checker page.
     *
     * @return string
     */
    private function size_checker_how_it_works_html() {
        $steps = array(
            ...
            __(
                'We rank the stone\'s face-up area — what you actually see when set — against real market inventory at that weight.',
                'loupe-diamond-network'
            ),
            __(
                'Optionally add a second diamond to see a true-scale overlay and per-carat spread comparison.',
                'loupe-diamond-network'
            ),
        );
        $out = '<ol class="ldn-size-checker-steps">';
        foreach ($steps as $step) {
            $out .= '<li>' . esc_html($step) . '</li>';
        }
        $out .= '</ol>';
        return '<div class="ldn-size-checker-how"><h2 class="ldn-size-checker-how__title">'
            . esc_html__('How it works', 'loupe-diamond-network') . '</h2>' . $out . '</div>';
    }

    /**
     * Example preview shown before the reader runs their first check (full tool only).
     *
     * @param array $summary size_checker manifest.
     * @return string
     */
    private function size_checker_empty_state_html(array $summary) {
        $default_a = isset($summary['default_a']) && is_array($summary['default_a'])
            ? $summary['default_a'] : array();
        $default_b = isset($summary['default_b']) && is_array($summary['default_b'])
            ? $summary['default_b'] : array();
        $shape_a = isset($default_a['shape']) ? ucwords(str_replace('-', ' ', (string) $default_a['shape'])) : 'Oval';
        $shape_b = isset($default_b['shape']) ? ucwords(str_replace('-', ' ', (string) $default_b['shape'])) : 'Oval';
        $carat_a = isset($default_a['carat']) ? (string) $default_a['carat'] : '1.1';
        $carat_b = isset($default_b['carat']) ? (string) $default_b['carat'] : '1.3';
        $lead = sprintf(
        ...
        return '<div class="ldn-size-checker-empty" id="ldn-size-checker-empty" aria-hidden="false">'
            . '<p class="ldn-size-checker-empty__lead">' . esc_html($lead) . '</p>'
            . '<div class="ldn-size-checker-empty__preview" id="ldn-size-checker-empty-preview"></div>'
            . '</div>';
    }

    /**
     * Cross-links below the checker on the full tool page.
     *
     * @param LDN_Page_Context $ctx
     * @return string
     */
    ...
    public function size_checker_body_html(LDN_Page_Context $ctx, array $summary, $widget = false) {
        ...
        $out = '<section class="ldn-section ldn-size-checker' . ($widget ? ' ldn-size-checker--widget' : ' ldn-size-checker--full') . '"'
            . ' id="ldn-size-checker"'
        ...
        $out .= '<div class="ldn-size-checker__surface">';

        if (!$widget) {
            $out .= $this->size_checker_how_it_works_html();
        }
        ...
        $out .= '<div class="ldn-size-checker-results" id="ldn-size-checker-results" aria-live="polite"'
            . ($widget ? ' hidden' : '') . '>';
        $out .= '<h2 class="ldn-size-checker-results__title">' . esc_html__('Results', 'loupe-diamond-network') . '</h2>';
        if (!$widget) {
            $out .= $this->size_checker_empty_state_html($summary);
        }
        ...
        if (!$widget) {
            ...
            $out .= $this->size_checker_explore_links_html($ctx);
        } else {
    ...
    private function comparison_body_html(LDN_Page_Context $ctx, array $summary) {
        ...
        $out = '';
        $callout = $this->comparison_callout_html($summary);
        if ($callout !== '') {
        ...
        if ($fa > 0 && $fb > 0) {
            $visual = '<section class="ldn-section ldn-size-comparison-visual"><h2>'
                . esc_html__('Face-up overlay', 'loupe-diamond-network') . '</h2>'
                . $this->comparison_legend_html($summary);
            if ($overlay !== '') {
                $visual .= '<div class="ldn-faceup-overlay ldn-size-outline ldn-size-outline--comparison">'
                    . $overlay . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            } else {
                $visual .= '<div id="ldn-comparison-overlay" class="ldn-faceup-overlay ldn-size-outline ldn-size-outline--comparison"'
                    . ' aria-busy="true" aria-label="'
                    . esc_attr__('Face-up size comparison', 'loupe-diamond-network') . '"></div>';
                $visual .= $this->comparison_overlay_data_script($summary);
                $visual .= $this->checker_manifest_script($ctx);
            }
            $bars = $this->comparison_faceup_bars_html($summary);
            if ($bars !== '') {
                $visual .= $bars;
            }
            $out .= $visual . '</section>';
     ...
     */
    /**
     * Market-price cards for both stones on a comparison page.
     *
     * @param LDN_Page_Context $ctx
     * @param array            $summary
     * @return string
     */
    public function comparison_price_links_html(LDN_Page_Context $ctx, array $summary) {
        if (!$this->config->size_price_internal_links($ctx->site_id)) {
            return '';
        }
        if ($ctx->country_code !== $this->config->size_rollout_country($ctx->site_id)) {
        ...
        foreach (array('a', 'b') as $key) {
            ...
            $cards = $this->comparison_price_cards_for_side($ctx, $side);
            if ($cards === array()) {
                continue;
            }
            $shape = ucwords(str_replace('-', ' ', (string) $side['shape']));
            $title = sprintf(
                /* translators: 1: carat weight, 2: shape name */
                __('%1$s carat %2$s', 'loupe-diamond-network'),
                (string) $side['carat'],
                $shape
            );
            $columns[] = '<div class="ldn-size-comparison-prices__stone"><h3>'
                . esc_html($title) . '</h3><div class="ldn-size-price-cards">'
                . implode('', $cards) . '</div></div>';
        }
        if ($columns === array()) {
            return '';
        }

        return '<section class="ldn-section ldn-size-comparison-prices"><h2>'
            . esc_html__('What do these diamonds cost?', 'loupe-diamond-network') . '</h2>'
            . '<div class="ldn-size-comparison-prices__grid">'
            . implode('', $columns)
            . '</div></section>';
    }

    /**
     * Natural + lab-grown price snapshot cards for one comparison side.
     *
     * @param LDN_Page_Context $ctx
     * @param array            $side
     * @return array<int, string>
     */
    private function comparison_price_cards_for_side(LDN_Page_Context $ctx, array $side) {
        $price_renderer = $this->price_renderer();
        $country = $this->config->size_rollout_country($ctx->site_id);
        $currency = $this->config->get_currency($ctx->site_id, $country);
        $carat = (float) ($side['carat'] ?? 0);
        $cards = array();
        ...
        foreach (array('natural', 'lab-grown') as $dtype) {
            ...
            $label = $dtype === 'lab-grown'
                ? __('Lab-grown', 'loupe-diamond-network')
                : __('Natural', 'loupe-diamond-network');
            $card = $this->comparison_price_snapshot_card_html($price_ctx, $url, $label, $currency, $carat);
            if ($card !== '') {
                $cards[] = $card;
            }
        }
    ...
    /**
     * Price snapshot card with optional per-carat line for comparison pages.
     *
     * @param LDN_Page_Context $price_ctx
     * @param string           $url
     * @param string           $label
     ...
     * @return string
     */
    private function comparison_price_snapshot_card_html(
        LDN_Page_Context $price_ctx,
        $url,
        $label,
        $currency,
        $carat
        ...
        $out .= '<span class="ldn-size-price-card__price">'
            . esc_html($price_renderer->format_stat($current, 'currency', $currency)) . '</span>';
        if ($carat > 0) {
            $per_carat = (float) $current / $carat;
            $out .= '<span class="ldn-size-price-card__per-carat">'
                . esc_html(sprintf(
                    /* translators: %s: formatted price per carat */
                    __('%s per carat', 'loupe-diamond-network'),
                    $price_renderer->format_stat($per_carat, 'currency', $currency)
                )) . '</span>';
        }
        $low = $this->dig_any($summary, array(
            array('distribution', 'price_range', 'min'),
    ...
    /**
     * JSON payload for client-side face-up overlay on long-tail comparison pages.
     *
     * @param array $summary
     * @return string
     */
    private function comparison_overlay_data_script(array $summary) {
        if (!isset($summary['a'], $summary['b'])) {
            return '';
        }
        $payload = array(
            'a' => array(
                'shape'     => (string) ($summary['a']['shape'] ?? ''),
                'width_mm'  => (float) ($summary['a']['width_mm'] ?? 0),
                'length_mm' => (float) ($summary['a']['length_mm'] ?? 0),
            ),
            'b' => array(
                'shape'     => (string) ($summary['b']['shape'] ?? ''),
                'width_mm'  => (float) ($summary['b']['width_mm'] ?? 0),
                'length_mm' => (float) ($summary['b']['length_mm'] ?? 0),
            ),
        );
        ...
        return '<script type="application/json" id="ldn-comparison-overlay-data">'
            . str_replace('</', '<\/', $json) . '</script>';
    }
    ...
    public function build_comparison_summary(array $a, array $b) {
        ...
        $visuals_a = isset($a['visuals']) && is_array($a['visuals']) ? $a['visuals'] : array();
        $visuals_b = isset($b['visuals']) && is_array($b['visuals']) ? $b['visuals'] : array();
        $scale_a = isset($visuals_a['scale_reference_svg']) && is_string($visuals_a['scale_reference_svg'])
            ? $visuals_a['scale_reference_svg'] : '';
        $scale_b = isset($visuals_b['scale_reference_svg']) && is_string($visuals_b['scale_reference_svg'])
            ? $visuals_b['scale_reference_svg'] : '';

        return array(
            'type'    => 'comparison',
            'a'       => $this->comparison_side_from_summary($a, $la, $wa, $fa),
            'b'       => $this->comparison_side_from_summary($b, $lb, $wb, $fb),
            'deltas'  => array(
                'faceup_area_pct' => $pct,
            ...
            'visuals' => array(
                'scale_reference_a_svg' => $scale_a,
                'scale_reference_b_svg' => $scale_b,
                'overlay_svg'           => '',
            ),
        );
    }
    ...
    /**
     * Normalise one individual size-summary into a comparison side payload.
     *
     * @param array    $summary
     * @param mixed    $length
     * @param mixed    $width
     ...
     * @return array
     */
    private function comparison_side_from_summary(array $summary, $length, $width, $faceup) {
        $ideal = isset($summary['ideal']) && is_array($summary['ideal']) ? $summary['ideal'] : array();
        $ideal_face = $ideal['faceup_area_mm2'] ?? null;
        $face_delta = null;
        if ($ideal_face !== null && $faceup !== null && (float) $ideal_face != 0.0) {
            $face_delta = round(((float) $faceup - (float) $ideal_face) / (float) $ideal_face * 100, 1);
        }
        ...
        return array(
            ...
            'n'                => (int) ($summary['n'] ?? 0),
            'depth_percent'    => $this->dig($summary, array('depth_percent', 'mean')),
            'table_percent'    => $this->dig($summary, array('table_percent', 'mean')),
            'lw_ratio'         => $this->dig($summary, array('lw_ratio', 'median')),
            'ideal'            => array(
                'length_mm'       => $ideal['length_mm'] ?? null,
                'width_mm'        => $ideal['width_mm'] ?? null,
                'depth_mm'        => $ideal['depth_mm'] ?? null,
                'faceup_area_mm2' => $ideal_face,
            ),
            'faceup_delta_pct' => $face_delta,
        );
    }
